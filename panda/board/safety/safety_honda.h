  ...
  if (valid) {
    ...
    if (!gas_interceptor_detected) {
      if (addr == 0x17C) {
        ...
        if (gas && !honda_gas_prev && (!(honda_hw == HONDA_BG_HW) || !(honda_hw == HONDA_BH_HW))) {
          controls_allowed = 0;
        }
        honda_gas_prev = gas;
      }
...
static int honda_tx_hook(CAN_FIFOMailBox_TypeDef *to_send) {
  ...
  int pedal_pressed = ((!(honda_hw == HONDA_BG_HW) || !(honda_hw == HONDA_BH_HW)) && honda_gas_prev) || (gas_interceptor_prev > HONDA_GAS_INTERCEPTOR_THRESHOLD) ||
  ...
  if (addr == 0x200) {
    if (!current_controls_allowed) {
      if ((!(honda_hw == HONDA_BG_HW) || !(honda_hw == HONDA_BH_HW)) && (GET_BYTE(to_send, 0) || GET_BYTE(to_send, 1))) {
        tx = 0;
      }
    }
  }
